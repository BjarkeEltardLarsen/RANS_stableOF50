...
void kEpsilonStab<BasicTurbulenceModel>::correctNut()
{
  this->nut_ = Cmu_*sqr(k_)/max(epsilon_,lambda2_*C2_/C1_*2*magSqr(symm(fvc::grad(this->U_)))/(2*magSqr(skew(fvc::grad(this->U_)))+pOmegaSmall_)*epsilon_); // nut as suggested by Larsen and Fuhrman (2018)
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> kEpsilonStab<BasicTurbulenceModel>::kSource() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> kEpsilonStab<BasicTurbulenceModel>::epsilonSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
template<class BasicTurbulenceModel>
kEpsilonStab<BasicTurbulenceModel>::kEpsilonStab
(
...
template<class BasicTurbulenceModel>
bool kEpsilonStab<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void kEpsilonStab<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
...
// Calculate eddy viscosity using the stabilizing approach described in Larsen and Fuhrman (2018)
 ...
 volScalarField epsilonTilde = max(epsilon_,lambda2_*C2_/C1_*p0*epsilon_/(pOmega+pOmegaSmall_));
