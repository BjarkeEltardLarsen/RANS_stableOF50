...
void RNGkEpsilonStab<BasicTurbulenceModel>::correctNut()
{
  this->nut_ = Cmu_*sqr(k_)/max(epsilon_,1.2603*lambda2_*C2_/C1_*2*magSqr(symm(fvc::grad(this->U_)))/(2*magSqr(skew(fvc::grad(this->U_)))+pOmegaSmall_)*epsilon_); //nut as suggested by Larsen and Fuhrman (2018);
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> RNGkEpsilonStab<BasicTurbulenceModel>::kSource() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> RNGkEpsilonStab<BasicTurbulenceModel>::epsilonSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
template<class BasicTurbulenceModel>
RNGkEpsilonStab<BasicTurbulenceModel>::RNGkEpsilonStab
(
...
template<class BasicTurbulenceModel>
bool RNGkEpsilonStab<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void RNGkEpsilonStab<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
...
// Calculate eddy viscosity using the stabilizing approach described in Larsen and Fuhrman (2018)
 ...
 volScalarField epsilonTilde = max(epsilon_,1.2603*lambda2_*C2_/C1_*p0*epsilon_/(pOmega+pOmegaSmall_)); 

  volScalarField nut  = Cmu_*sqr(k_)/epsilonTilde;
