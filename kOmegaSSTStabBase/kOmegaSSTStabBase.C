...
{
  this->nut_ = a1_*k_/max(a1_*lambda2_*beta1_/(betaStar_*gamma1_)*2*magSqr(symm(fvc::grad(this->U_)))/(2*magSqr(skew(fvc::grad(this->U_)))+pOmegaSmall_)*omega_,max(a1_*omega_, b1_*F2*sqrt(S2))); //nut as suggested by Larsen and Fuhrman 2018
